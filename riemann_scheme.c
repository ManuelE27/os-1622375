...
void* calcular(void *arg) {
	...
	arg=MAXRECT/arg; //porcion de rectangulos por hilo
	double base_length=1/hilos;
	int loslimit;
...
int main(int argc, char** argv) {
	...
	if (argc != 2) {
		printf("Por favor digite cuantos rectangulos desea por hilo: \n");
		exit(-1);
	}
	else {
...
//	pthread_t threads;
	pthread_t threads[hilos]; // almacena 'id' de hilos
	...
	base_length = 1/MAXRECT; // esta variable contiene el ancho de
